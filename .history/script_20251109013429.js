...
/* === View Product Image: toggle + add "Get a Quote" button + lightbox === */
document.addEventListener("DOMContentLoaded", function () {
  const button = document.getElementById("openQuickpatchArticle");   // your existing button
  const container = document.getElementById("productImageContainer"); // your existing container

  if (!button || !container) return;

  // add lightbox behavior to the images already inside the container
  function wireLightbox() {
    container.querySelectorAll("img").forEach((img) => {
      ...
      img.addEventListener("click", () => {
        // openLightbox is already defined earlier in your script
        openLightbox(img.src, img.alt || "Product image");
      });
    });
  }

  // add a single "Get a Quote" button after the images
  function ensureQuoteButton() {
    if (container.querySelector("#quoteAfterImageBtn")) return;
    const quoteBtn = document.createElement("button");
    quoteBtn.id = "quoteAfterImageBtn";
  ...
  // toggle visibility and update button label
