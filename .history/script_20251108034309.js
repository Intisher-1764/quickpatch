...
document.addEventListener("DOMContentLoaded", function() {
      ...
      button.addEventListener("click", function() {
        imgContainer.style.display =
          (imgContainer.style.display === "none" || imgContainer.style.display === "")
            ? "block"
            : "none";
      });
    });
    document.addEventListener('DOMContentLoaded', () => {
  const toggleBtn = document.getElementById('openQuickpatchArticle');
  const container = document.getElementById('productImageContainer');

  // Ensure initial hidden state but height reserved by CSS
  container.classList.remove('show');

  toggleBtn.addEventListener('click', () => {
    const isOpen = container.classList.toggle('show');
    // swap label for better UX
    toggleBtn.textContent = isOpen ? 'Hide product image' : 'View product image';
